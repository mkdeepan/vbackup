...
//var_dump($tips); exit;
        $dbCon = null;
       // $tips[0]->atHome = str_replace("\r\n","",strip_tags($tips[0]->atHome));
        // $tips[0]->atSchool = str_replace("\r\n","",strip_tags($tips[0]->atSchool));
    ...
    $input = json_decode($body); 
	 ...
	 $errorCodes = array();
		if(!empty($input)){
		  	 ...
		  	 {
		  	 	$errorCodes = array('statusCode'=>'111','errorMessage'=>'API parameter missing');
		  	   goto res;
		  	 }
		       ...
		       if(!filter_var($logDetails['lat'], FILTER_VALIDATE_FLOAT) || !filter_var($logDetails['long'], FILTER_VALIDATE_FLOAT) || !($logDetails['lat'] >= -90 && $logDetails['lat'] <= 90) || !($logDetails['long'] >= -180 && $logDetails['long'] <= 180)){
			    ...
			    try {
			        ...
			        if($user){
			            ...
			            $errorCodes = array('statusCode'=>'109','errorMessage'=>'User not found');
  	                  goto res;
			        }
			    }catch(PDOException $e) {
...
$app->post('/signUp', function(){
	global $app;
        ...
        $input = json_decode($body);
        $base_url = 'http://myvalert.com/';
        //var_dump($input); echo $input->signup->accountFirstName; exit;
      if(!empty($input)){
        $error = 0;
        $errorCodes = array(); 
